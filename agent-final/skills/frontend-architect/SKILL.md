...
### 3. Responsive & Layout ([PROJECT_NAME] Context)
- **Responsive Sizing:** ALWAYS use `flutter_screenutil` extensions (`.w`, `.h`, `.sp`, `.r`) instead of hardcoded raw pixels for dimensions, fonts, and radii.
- **Layout Safety:** Wrap critical content in `SafeArea` to avoid system UI overlaps.
- **Overflow Prevention:** Proactively use `SingleChildScrollView`, `ListView`, or `Expanded` to prevent layout overflow errors on smaller screens or when keyboard appears.
