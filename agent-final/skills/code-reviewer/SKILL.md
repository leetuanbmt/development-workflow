...
## 🛠 Included Scripts
- **Diff Generator:** `.agent/skills/code-reviewer/scripts/create_pr_diff.sh`
  - Purpose: Auto-detects base branch and generates a clean `pr_changes.diff`.
  - Usage: `./.agent/skills/code-reviewer/scripts/create_pr_diff.sh [base_branch] [-y]`

## 🧠 Golden Rule: Understand First
*   **Diff Source:** Always check if `pr_changes.diff` exists in the root (generated by `/review` workflow). Prioritize reading it over checking raw git diffs manually to ensure clean context.
...
### 1. Architecture Compliance ([PROJECT_NAME])
- [ ] Layer separation clear (Data/Domain/Presentation)?
- [ ] No circular dependencies?
- [ ] Domain layer pure (no external framework dependencies)?
- [ ] **Data vs Domain:** Model (DTO) logic does NOT leak into Presentation?
- [ ] **Dependency Injection:** Dependencies injected via Constructor (`@injectable`) instead of direct `GetIt.I` calls (unless entry point/factory)?

### 2. Business Logic
- [ ] Logic matches Spec?
- [ ] Edge cases handled?
- [ ] Input validation present?
- [ ] **Asynchronous:** Uses `async/await` instead of `.then()`? Try-catch implemented at Data/Presentation layer?
